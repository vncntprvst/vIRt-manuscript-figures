    ...
    if savePlots
        ...
        [spS.spectrumVals,spS.time,spS.freqVals]=...
            ...
            for wEpochNum=1:numEpochs
                ...
                imagesc(spS.time,spS.freqVals,10*log10(squeeze(spS.spectrumVals(:,:,wEpochNum)))');
                xline(2,'LineWidth',2,'Color',[0 0 0 0.5])
                axis('tight');box off;
                xlabel('Time (ms)')
                ylabel('Frequency (Hz)');
                ...
                disp(['epoch num #' num2str(wEpochNum) ' starts at t=' num2str(epochIdx(1))])
                plot(wAngle(epochIdx(1)-2000:epochIdx(3000)))
                %         plot(spikeTimes(wEpochNum).transition*1000,...
                %             ones(1,numel(spikeTimes(wEpochNum).transition))*max(wAngle)+20,'kd')
                xline(2000,'LineWidth',2,'Color',[0 0 0 0.5])
                axis('tight');box off;
                xlabel('Time (ms)')
                ylabel('Angle (d)');
                set(gca,'xlim',[0 5000],'xticklabels',-2000:500:5000,...
                    'FontSize',10,'FontName','Helvetica','TickDir','out','Color','white');%'Calibri'
            end
        end
